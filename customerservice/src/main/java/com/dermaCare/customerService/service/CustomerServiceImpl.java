...
/**
 * NOTE: This class relies on @EnableScheduling being present somewhere in the
 * application context (e.g. on the main @SpringBootApplication class) for the
 * cleanupExpiredOtps() scheduled task below to actually run.
 */
@Service
public class CustomerServiceImpl implements CustomerService {
    ...
    // FIX: this was never annotated with @Autowired in the original file, so it
    // was always null. Every call site wrapped its usage in an empty
    // catch(Exception e) {}, which silently swallowed the resulting NPE and
    // meant therapy-answer question text was never being populated.
    @Autowired
    ...
    // FIX: switched from HashMap to ConcurrentHashMap. These maps are mutated by
    // request-handling threads AND by the new scheduled cleanup thread below, so
    // a plain HashMap is not safe here (risk of corruption / infinite loop under
    // concurrent modification).
    private Map<String, String> generatedOtps = new ConcurrentHashMap<>();
    private Map<String, Long> session = new ConcurrentHashMap<>();
    private static final long OTP_EXPIRY_MILLIS = 1 * 60 * 1000;

    /**
     * Periodically evicts expired OTP/session entries so these maps don't grow
     * unbounded while the service is idle. Runs every 60s.
     */
    @Scheduled(fixedRate = 60000)
    public void cleanupExpiredOtps() {
        long now = System.currentTimeMillis();
        session.entrySet().removeIf(e -> now - e.getValue() > OTP_EXPIRY_MILLIS);
        generatedOtps.keySet().removeIf(k -> !session.containsKey(k));
    }
    ...
    public Response getCustomerByMobileNumber(String mblnumber) {
        Response response = new Response();
        try {
            // FIX: original code called cusmr.get() inside the log statement BEFORE
            // checking isPresent(), which threw NoSuchElementException for any
            // mobile number not found in the DB instead of returning the intended
            // "no customer found" response.
            Optional<Customer> cusmr = customerRepository.findByMobileNumber(mblnumber);
            if (cusmr.isPresent()) {
                Customer c = cusmr.get();
                log.info("customer found with mobileNumber {}", c.getMobileNumber());
                ...
                log.warn("No customer found with mobileNumber {}", mblnumber);
        ...
        try {
            ...
            if (cusmr.isPresent()) {
                ...
                if (obj != null) {
                    ...
                    // FIX: original code had no else branch here. If save() returned
                    // null, execution fell through both branches and returned a
                    // blank, unset Response (status 0, no message).
                    response.setMessage("Unable to update details");
                    response.setStatus(500);
                    response.setSuccess(false);
                    return response;
                }
        ...
        try {
            ...
            if (c.isPresent()) {
                ...
                if (obj != null) {
                    ...
                    response.setMessage("Unable to delete customer");
                    response.setStatus(500);
                    response.setSuccess(false);
                    return response;
                }
    ...
    public CustomerDTO getCustomerDetailsByMobileNumber(String mobileNumber) {
        ...
        CustomerDTO customerDTO = new CustomerDTO(
                Optional.ofNullable(customerObject.getCustomerId()).orElse(null),
                Optional.ofNullable(customerObject.getFullName()).orElse(null),
                String.valueOf(customerObject.getMobileNumber()), null,
                Optional.ofNullable(customerObject.getGender()).orElse(null),
                Optional.ofNullable(customerObject.getEmailId()).orElse(null),
                null,
                Optional.ofNullable(customerObject.getDateOfBirth()).orElse(null));
        log.info("Customer found for mobile number: {}", mobileNumber);
        return customerDTO;
    }
    ...
    public CustomerDTO getCustomerDetailsByEmail(String email) {
        ...
        CustomerDTO customerDTO = new CustomerDTO(
                ...
                null,
                Optional.ofNullable(customerObject.getDateOfBirth()).orElse(null));
        log.info("Customer found for email: {}", email);
        return customerDTO;
    }
    ...
    public Response saveConsultation(ConsultationDTO dto) {
        ...
        try {
            ...
            // FIX: original condition was `duplicateConsult != null || !type.equals(" ")`,
            // which is true in almost every real case (any non-blank type satisfies
            // the second clause on its own), so duplicate consultation types were
            // being saved anyway. It's now: only save when there is NO existing
            // duplicate AND the type is non-blank. Also added the missing `return`
            // below -- previously, the "already exists" response was being silently
            // overwritten by the success block further down, because the local
            // `consultation` variable was never null (just an empty, unsaved entity),
            // so the `if (consultation != null)` check always passed.
            boolean typeIsBlank = dto.getConsultationType() == null || dto.getConsultationType().trim().isEmpty();
            if (duplicateConsult == null && !typeIsBlank) {
                ...
                log.info("SAVE_CONSULTATION :: SKIPPED_DUPLICATE_OR_EMPTY :: consultationId={}",
            ...
            if (consultation != null) {
                log.info("SAVE_CONSULTATION :: SUCCESS :: consultationId={}", consultation.getConsultationId());
    ...
    public Response getDoctors(String cid, String serviceId) {
        ...
        try {
            ...
            log.error("GET_DOCTORS :: FEIGN_ERROR :: clinicId={}, serviceId={}", cid, serviceId, e);
    ...
    public ResponseEntity<Response> saveFavouriteDoctors(FavouriteDoctorsDTO favouriteDoctorsDTO) {
        ...
        try {
            ...
            if (f != null) {
                ...
                log.warn("SAVE_FAVOURITE_DOCTOR :: FAILED :: doctorId={}", favouriteDoctorsDTO.getDoctorId());
            ...
            log.error("SAVE_FAVOURITE_DOCTOR :: ERROR :: doctorId={}", favouriteDoctorsDTO.getDoctorId(), e);
    ...
    public Response getDoctorsSlots(String hid, String branchId, String doctorId) {
        ...
        log.info("GET_DOCTOR_SLOTS :: START :: hospitalId={}, branchId={}, doctorId={}", hid, branchId, doctorId);
        ...
        try {
            ...
            log.info("GET_DOCTOR_SLOTS :: SUCCESS :: hospitalId={}, branchId={}, doctorId={}", hid, branchId,
                    doctorId);

            return res.getBody();

        } catch (FeignException e) {
            log.error("GET_DOCTOR_SLOTS :: FEIGN_ERROR :: hospitalId={}, branchId={}, doctorId={}", hid, branchId,
                    doctorId, e);
    ...
    public Response getReportsAndDoctorSaveDetails(String customerId) {
        ...
        try {
            ...
            // FIX: guard against a null Feign response body before dereferencing it.
            if (res == null) {
                log.warn("GET_REPORTS_AND_DOCTOR_DETAILS :: NULL_REPORTS_RESPONSE :: customerId={}", customerId);
                response.setStatus(200);
                response.setMessage("No reports data returned");
                response.setSuccess(true);
                return response;
            }

            List<ReportsDtoList> repots = new ObjectMapper().convertValue(res.getData(),
                    new TypeReference<List<ReportsDtoList>>() {
                    });

            log.info("GET_REPORTS_AND_DOCTOR_DETAILS :: REPORTS_COUNT :: customerId={}, count={}", customerId,
                    repots != null ? repots.size() : 0);
            ...
            List<DoctorSaveDetailsDTO> doctorSaveDetailsDTO = new ArrayList<>();
            if (rs != null) {
                log.debug("GET_REPORTS_AND_DOCTOR_DETAILS :: DOCTOR_DETAILS_RESPONSE_RECEIVED :: customerId={}",
                        customerId);
                ...
                doctorSaveDetailsDTO = mapper.convertValue(rs.getData(),
                        new TypeReference<List<DoctorSaveDetailsDTO>>() {
                        });
            } else {
                log.warn("GET_REPORTS_AND_DOCTOR_DETAILS :: NULL_DOCTOR_DETAILS_RESPONSE :: customerId={}",
                        customerId);
            }

            log.info("GET_REPORTS_AND_DOCTOR_DETAILS :: DOCTOR_DETAILS_COUNT :: customerId={}, count={}", customerId,
                    doctorSaveDetailsDTO != null ? doctorSaveDetailsDTO.size() : 0);
            ...
            if (doctorSaveDetailsDTO != null && !doctorSaveDetailsDTO.isEmpty()) {
                log.debug("GET_REPORTS_AND_DOCTOR_DETAILS :: SETTING_DOCTOR_DETAILS_DATA :: customerId={}",
                        customerId);
            ...
            log.error("GET_REPORTS_AND_DOCTOR_DETAILS :: FEIGN_ERROR :: customerId={}, status={}", customerId,
                    e.status(), e);
    ...
    public Response getCustomerBookedServices(String mobileNumber) {
        ...
        try {
            ResponseEntity<ResponseStructure<List<BookingResponse>>> res = bookingFeign
                    .getCustomerBookedServices(mobileNumber);
    ...
    public ResponseStructure<List<BookingResponse>> getAllBookedServices() {
        ...
        try {
            ...
            // FIX: original code dereferenced res.getData() without checking res
            // was non-null first -- a null Feign response body would NPE here.
            if (res == null) {
                log.warn("GET_ALL_BOOKINGS :: NULL_RESPONSE_BODY");
                return new ResponseStructure<>(null, "Bookings Not Found", HttpStatus.OK, HttpStatus.OK.value());
            }
            ...
            if (res.getData() != null && !res.getData().isEmpty()) {
                ...
                return new ResponseStructure<>(res.getData(), res.getMessage(), res.getHttpStatus(),
                        res.getStatusCode());
            } else {

                log.warn("GET_ALL_BOOKINGS :: EMPTY");

                return new ResponseStructure<>(null, "Bookings Not Found", res.getHttpStatus(), res.getStatusCode());
            }

        } catch (FeignException e) {
            log.error("GET_ALL_BOOKINGS :: FEIGN_ERROR", e);

            return new ResponseStructure<>(null, ExtractFeignMessage.clearMessage(e), HttpStatus.INTERNAL_SERVER_ERROR,
                    e.status());
        }
    }
    ...
    public Response getBookingByServiceId(String serviceId) {
        ...
        try {
            ResponseEntity<ResponseStructure<List<BookingResponse>>> res = bookingFeign
                    .getBookingByServiceId(serviceId);
    ...
    public Response submitCustomerRating(CustomerRatingDomain ratingRequest) {
        ...
        log.info("SUBMIT_RATING :: START :: branchId={}, doctorId={}, appointmentId={}",
                ratingRequest.getBranchId(), ratingRequest.getDoctorId(), ratingRequest.getAppointmentId());
        ...
        try {
            ...
            CustomerRating customerRating = customerRatingRepository.findByBranchIdAndDoctorIdAndAppointmentId(
                    ratingRequest.getBranchId(), ratingRequest.getDoctorId(), ratingRequest.getAppointmentId());

            if (customerRating == null) {

                log.info("SUBMIT_RATING :: NEW_RATING :: doctorRating={}, branchRating={}",
                        ratingRequest.getDoctorRating(), ratingRequest.getBranchRating());

                CustomerRating cRating = new CustomerRating(null, ratingRequest.getDoctorRating(),
                        ratingRequest.getBranchRating(), ratingRequest.getFeedback(), ratingRequest.getHospitalId(),
                        ratingRequest.getBranchId(), ratingRequest.getDoctorId(),
                        ratingRequest.getCustomerMobileNumber(), ratingRequest.getPatientId(),
                        ratingRequest.getPatientName(), ratingRequest.getAppointmentId(), true, formattedTime);
            ...
            log.info("SUBMIT_RATING :: COMPLETED :: branchId={}, doctorId={}", ratingRequest.getBranchId(),
                    ratingRequest.getDoctorId());

            return response;

        } catch (Exception e) {
            log.error("SUBMIT_RATING :: ERROR :: branchId={}, doctorId={}", ratingRequest.getBranchId(),
                    ratingRequest.getDoctorId(), e);
    ...
    public void updateAvgRatingInClinicAndDoctorObject(String bId, String doctorId) {
        ...
        try {
            ...
            log.debug("UPDATE_AVG_RATING :: RATINGS_COUNT :: branchRatings={}, doctorRatings={}",
                    clinicRatings.size(), doctorRatings.size());

            double avgClinicRating = clinicRatings.stream().mapToDouble(CustomerRating::getBranchRating).average()
                    .orElse(0.0);

            double avgDoctorRating = doctorRatings.stream().mapToDouble(CustomerRating::getDoctorRating).average()
                    .orElse(0.0);

            log.info("UPDATE_AVG_RATING :: CALCULATED :: branchAvg={}, doctorAvg={}", avgClinicRating,
                    avgDoctorRating);

            Response res = adminFeign.getBranchById(bId).getBody();
            if (res == null || res.getData() == null) {
                log.warn("UPDATE_AVG_RATING :: NULL_BRANCH_RESPONSE :: branchId={}", bId);
                return;
            }
            ...
            if (doctorsDTO.getBody() == null || doctorsDTO.getBody().getData() == null) {
                log.warn("UPDATE_AVG_RATING :: NULL_DOCTOR_RESPONSE :: doctorId={}", doctorId);
                return;
            }
            ...
            log.error("UPDATE_AVG_RATING :: FEIGN_ERROR :: branchId={}, doctorId={}", bId, doctorId, e);
        }
    }
        ...
        try {
            ...
            for (CustomerRating rating : ratings) {
                CustomerRatingDomain c = new CustomerRatingDomain(rating.getDoctorRating(), rating.getBranchRating(),
                        rating.getFeedback(), rating.getHospitalId(), rating.getBranchId(), rating.getDoctorId(),
                        rating.getCustomerMobileNumber(), rating.getPatientId(), rating.getPatientName(),
                        rating.getAppointmentId(), rating.getRated(), rating.getDateAndTimeAtRating());
                listDto.add(c);
            }
    ...
    public Response getAverageRating(String branchId, String doctorId) {
        ...
        try {
            ...
            log.error("GET_AVERAGE_RATING :: FEIGN_ERROR :: branchId={}, doctorId={}", branchId, doctorId, e);
    ...
    public Response getDoctorsandHospitalDetails(String hospitalId, String subServiceId)
        ...
        log.info("GET_DOCTORS_AND_HOSPITAL :: START :: hospitalId={}, subServiceId={}", hospitalId, subServiceId);
        ...
        try {
            ...
            if (hospitalResponse.getData() != null) {
                ...
                ResponseEntity<Response> doctorsResponse = clinicAdminFeign.getDoctorsBySubServiceId(hospitalId,
                        subServiceId);

                Object obj = doctorsResponse.getBody().getData();

                List<DoctorsDTO> doctors = new ObjectMapper().convertValue(obj,
                        new TypeReference<List<DoctorsDTO>>() {
                        });

                log.info("GET_DOCTORS_AND_HOSPITAL :: DOCTORS_COUNT :: {}", doctors != null ? doctors.size() : 0);
            ...
            log.error("GET_DOCTORS_AND_HOSPITAL :: FEIGN_ERROR :: hospitalId={}, subServiceId={}", hospitalId,
                    subServiceId, e);
    ...
    public Response getHospitalsAndDoctorsDetailsBySubServiceId(String subServiceId) {
        ...
        try {
            ResponseEntity<Response> doctorsResponse = clinicAdminFeign
                    .getHospitalAndDoctorUsingSubServiceId(subServiceId);
            ...
            log.error("GET_HOSPITALS_AND_DOCTORS_BY_SUBSERVICE :: FEIGN_ERROR :: subServiceId={}", subServiceId, e);
    ...
    public ResponseEntity<?> getInProgressAppointments(String mnumber) {
        ...
        try {
            ...
            ResBody<List<NotificationToCustomer>> res = new ResBody<>(ExtractFeignMessage.clearMessage(e), e.status(),
                    null);
    ...
    public ResponseEntity<?> customerLogin(CustomerLoginDTO dto) {
        ...
        try {
            ...
            log.error("CUSTOMER_LOGIN :: FEIGN_ERROR :: username={}", dto.getUserName(), e);
    ...
    public Response getDoctorsByHospitalBranchAndSubService(String hospitalId, String branchId, String subServiceId)
        ...
        log.info(
                "GET_DOCTORS_BY_HOSPITAL_BRANCH_SUBSERVICE :: START :: hospitalId={}, branchId={}, subServiceId={}",
        ...
        try {
            ...
            if (hospitalResponse.getData() != null) {
                ...
                log.debug("GET_DOCTORS_BY_HOSPITAL_BRANCH_SUBSERVICE :: HOSPITAL_FOUND :: hospitalId={}", hospitalId);

                ResponseEntity<Response> doctorsResponse = clinicAdminFeign
                        .getDoctorsByHospitalBranchAndSubService(hospitalId, branchId, subServiceId);
            ...
            log.error(
                    "GET_DOCTORS_BY_HOSPITAL_BRANCH_SUBSERVICE :: FEIGN_ERROR :: hospitalId={}, branchId={}, subServiceId={}",
    ...
    public Response getDoctorsByHospitalBranchAndSubService(String hospitalId, String branchId, String subServiceId,
        ...
        log.info(
                "GET_DOCTORS_BY_HOSPITAL_BRANCH_SUBSERVICE_CT :: START :: hospitalId={}, branchId={}, subServiceId={}, consultationType={}",
        ...
        try {
            ...
            if (hospitalResponse.getData() != null) {
                ...
                if (!doctors.isEmpty()) {
                    ...
                    List<DoctorsDTO> filteredDoctors = doctors.stream().filter(dto -> {
            ...
            log.error("GET_DOCTORS_BY_HOSPITAL_BRANCH_SUBSERVICE_CT :: FEIGN_ERROR", e);
    ...
    public ResponseEntity<ResponseStructure<List<BookingResponse>>> getBookingsByClinicIdWithBranchId(
        ...
        log.info("GET_BOOKINGS_BY_CLINIC_BRANCH :: START :: clinicId={}, branchId={}", clinicId, branchId);
    ...
    @Override
    public ResponseEntity<ResponseStructure<List<Map<String, Object>>>> getBookingsByCustomerId(String customerId) {

        log.info("GET_BOOKINGS_BY_CUSTOMER :: START :: customerId={}", customerId);

        try {

            ResponseEntity<ResponseStructure<List<Map<String, Object>>>> response = bookingFeign
            ...
            if (response == null || response.getBody() == null) {
                ...
                return ResponseEntity.status(HttpStatus.NO_CONTENT).body(ResponseStructure.buildResponse(null,
                        "No booking data found.", HttpStatus.NO_CONTENT, HttpStatus.NO_CONTENT.value()));
            }
            ...
            return ResponseEntity.status(status)
                    .body(ResponseStructure.buildResponse(null, ExtractFeignMessage.clearMessage(e), status,
                            status.value()));
    ...
    public ResponseEntity<?> getCompletedBookingsByCustomerId(String customerId) {
        ...
        try {
            ...
            log.error("GET_BOOKINGS_BY_CUSTOMER :: FEIGN_ERROR :: customerId={}", customerId, e);
    ...
    public ResponseEntity<?> getInprogressBookingsByCustomerId(String customerId) {
        ...
        try {
            ResponseEntity<?> response = bookingFeign.getInprogressAppointmentsByCustomerId(customerId);
            ...
            log.error("GET_INPROGRESS_BOOKINGS_BY_CUSTOMER :: FEIGN_ERROR :: customerId={}", customerId, e);
    ...
    public ResponseEntity<?> getInprogressBookingsByPatientId(String patientId, String clinicId) {
        ...
        log.info("GET_INPROGRESS_BOOKINGS_BY_PATIENT :: START :: patientId={}, clinicId={}", patientId, clinicId);

        ResponseStructure<List<BookingResponse>> res = new ResponseStructure<>();

        try {
            ResponseEntity<?> response = bookingFeign.getInprogressAppointmentsByPatientId(patientId, clinicId);
            ...
            log.error("GET_INPROGRESS_BOOKINGS_BY_PATIENT :: FEIGN_ERROR :: patientId={}, clinicId={}", patientId,
                    clinicId, e);
    ...
    public ResponseEntity<?> retrieveAppointnmentsByRelation(String customerId) {
        ...
        try {
            ResponseEntity<?> response = bookingFeign.retrieveAppointnmentsByRelation(customerId);
            ...
            log.error("RETRIEVE_APPOINTMENTS_BY_RELATION :: FEIGN_ERROR :: customerId={}", customerId, e);
    ...
    public ResponseEntity<?> retrieveAppointnmentsByPatientId(String patientId) {
        ...
        try {
            ResponseEntity<?> response = bookingFeign.getBookingByPatientId(patientId);
            ...
            log.error("RETRIEVE_APPOINTMENTS_BY_PATIENT :: FEIGN_ERROR :: patientId={}", patientId, e);
    ...
    public boolean blockSlot(TempBlockingSlot tempBlockingSlot) {
        ...
        log.info("BLOCK_SLOT :: START :: doctorId={}, branchId={}", tempBlockingSlot.getDoctorId(),
        ...
        try {
            ...
            log.error("BLOCK_SLOT :: FEIGN_ERROR :: doctorId={}, branchId={}", tempBlockingSlot.getDoctorId(),
                    tempBlockingSlot.getBranchId(), e);
    ...
    public CustomerDTO getCustomerByToken(String token) {
        ...
        try {
            ...
            if (cstmr != null) {
                ...
                log.info("GET_CUSTOMER_BY_TOKEN :: FOUND :: customerId={}", cstmr.getCustomerId());
            ...
            if (req.getTheraphyAnswers() != null && !req.getTheraphyAnswers().isEmpty()) {
                ...
                for (Map.Entry<String, List<TheraphyAnswersDTO>> entry : map.entrySet()) {
                    ...
                    try {
                        ...
                        log.warn("BOOK_PHYSIO :: FAILED_TO_RESOLVE_QUESTIONS_FOR_KEY :: key={}", key, e);
                    }
                    if (entity == null || entity.getQuestionsByPart() == null) {
                        continue;
                    }
                ...
                if (req != null) {
                    clinicAdminFeign.updateDoctorSlotWhileBooking(req.getDoctorId(), req.getBranchId(),
                            req.getServiceDate(), req.getServicetime());
                }
    ...
    public ResponseEntity<ResBody<List<NotificationToCustomer>>> notificationToCustomer(String customerMobileNumber) {
        ...
        try {
            ...
            log.error("CUSTOMER_NOTIFICATION :: FEIGN_ERROR :: mobileNumber={}", customerMobileNumber, e);
    ...
    @Override
    public ResponseEntity<Response> getStaffInfo(String hospitalId, String branchId) {
        Response response = new Response();
        try {
    ...
    @Override
    public ResponseEntity<Response> getByClinicIdAndBranchId(String clinicId, String branchId, String patientId) {
        Response res = new Response();
        try {
