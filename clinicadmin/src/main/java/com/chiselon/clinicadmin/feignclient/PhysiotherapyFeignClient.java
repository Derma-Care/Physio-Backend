...
public interface PhysiotherapyFeignClient {
    ...
    void updateSessionStatus(
    		@RequestHeader("Authorization") String token,@PathVariable("therapistRecordId") String therapistRecordId,
            @PathVariable("sessionId") String sessionId
    );

    @GetMapping("/api/physiotherapy-doctor/payment/{bookingId}")
    Response getPayment(
    		@RequestHeader("Authorization") String token,  @PathVariable("bookingId") String bookingId
    );
