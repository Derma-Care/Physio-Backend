...
public class DoctorPrescriptionServiceImpl implements DoctorPrescriptionService {
	...
	public Response createPrescription(DoctorPrescriptionDTO dto) {
		try {
			...
			if (dto == null || dto.getMedicines() == null || dto.getMedicines().isEmpty()) {
				return new Response(false, null, "Prescription must have at least one medicine",
			...
			DoctorPrescription entity = prescriptions.isEmpty() ? new DoctorPrescription() : prescriptions.get(0); // only
																													// one
																													// prescription
																													// per
																													// clinic
			...
			List<Medicine> existingMedicines = Optional.ofNullable(entity.getMedicines()).orElse(new ArrayList<>());
			...
			for (MedicineDTO incomingMed : dto.getMedicines()) {
				...
				if (existingMedOpt.isPresent()) {
					...
					// Add new medicine
					existingMedicines.add(new Medicine(UUID.randomUUID().toString(), incomingMed.getName().trim(),
							incomingMed.getDose(), incomingMed.getDuration(), incomingMed.getDurationUnit(),
							incomingMed.getNote(), incomingMed.getFood(), incomingMed.getMedicineType(),
							incomingMed.getRemindWhen(), incomingMed.getOthers(), incomingMed.getTimes(),
							incomingMed.getSerialNumber(), incomingMed.getGenericName(), incomingMed.getBrandName(),
							incomingMed.getNameAndAddressOfTheManufacturer(), incomingMed.getBatchNumber(),
							incomingMed.getDateOfManufacturing(), incomingMed.getDateOfExpriy(),
							incomingMed.getManufacturingLicenseNumber(), incomingMed.getStock()));
			...
			responseDTO.setMedicines(saved.getMedicines().stream()
					.map(m -> new MedicineDTO(m.getId(), m.getName(), m.getDose(), m.getDuration(), m.getDurationUnit(),
							m.getNote(), m.getFood(), m.getMedicineType(), m.getRemindWhen(), m.getTimes(),
							m.getOthers(), m.getSerialNumber(), m.getGenericName(), m.getBrandName(),
							m.getNameAndAddressOfTheManufacturer(), m.getBatchNumber(), m.getDateOfManufacturing(),
							m.getDateOfExpriy(), m.getManufacturingLicenseNumber(), m.getStock()))
		...
		try {
			List<DoctorPrescriptionDTO> dtos = repository.findAll().stream().map(p -> {
				...
				List<MedicineDTO> meds = Optional.ofNullable(p.getMedicines()).orElse(List.of()).stream()
						.map(m -> new MedicineDTO(m.getId(), m.getName(), m.getDose(), m.getDuration(),
								m.getDurationUnit(), m.getNote(), m.getFood(), m.getMedicineType(), m.getRemindWhen(),
								m.getTimes(), m.getOthers(), m.getSerialNumber(), m.getGenericName(), m.getBrandName(),
								m.getNameAndAddressOfTheManufacturer(), m.getBatchNumber(), m.getDateOfManufacturing(),
								m.getDateOfExpriy(), m.getManufacturingLicenseNumber(), m.getStock()))
						.collect(Collectors.toList());
	...
	public Response getMedicineById(String medicineId) {
		try {
			List<Medicine> matches = repository.findAll().stream()
					...
					.filter(m -> m.getId() != null && m.getId().equals(medicineId)).distinct()
					.collect(Collectors.toList());

			if (!matches.isEmpty()) {
				List<MedicineDTO> dtos = matches.stream()
						.map(m -> new MedicineDTO(m.getId(), m.getName(), m.getDose(), m.getDuration(),
								m.getDurationUnit(), m.getNote(), m.getFood(), m.getMedicineType(), m.getRemindWhen(),
								m.getTimes(), m.getOthers(), m.getSerialNumber(), m.getGenericName(), m.getBrandName(),
								m.getNameAndAddressOfTheManufacturer(), m.getBatchNumber(), m.getDateOfManufacturing(),
	...
	public Response searchMedicinesByName(String keyword) {
		try {
			...
			if (latestMedicine.isEmpty()) {
				return new Response(false, null, "No medicine found with exact name: " + keyword,
						HttpStatus.NOT_FOUND.value());
			}

			Medicine m = latestMedicine.get();
			MedicineDTO dto = new MedicineDTO(m.getId(), m.getName(), m.getDose(), m.getDuration(), m.getDurationUnit(),
					m.getNote(), m.getFood(), m.getMedicineType(), m.getRemindWhen(), m.getTimes(), m.getOthers(),
					m.getSerialNumber(), m.getGenericName(), m.getBrandName(), m.getNameAndAddressOfTheManufacturer(),
					m.getBatchNumber(), m.getDateOfManufacturing(), m.getDateOfExpriy(),
					m.getManufacturingLicenseNumber(), m.getStock());
	...
	public Response updatePrescription(String id, DoctorPrescriptionDTO dto) {
		try {
			Optional<DoctorPrescription> optional = repository.findById(id);
			if (optional.isEmpty()) {
				return new Response(false, null, "Prescription not found with id: " + id, HttpStatus.NOT_FOUND.value());
			}
				...
				for (MedicineDTO medDto : dto.getMedicines()) {
					if (medDto.getId() != null) {
						...
						Optional<Medicine> existingMedOpt = existingPrescription.getMedicines().stream()
								.filter(m -> m.getId().equals(medDto.getId())).findFirst();
						...
						if (existingMedOpt.isPresent()) {
							...
							updatedMedicines.add(new Medicine(
									...
									medDto.getName(), medDto.getDose(), medDto.getDuration(), medDto.getDurationUnit(),
									medDto.getNote(), medDto.getFood(), medDto.getMedicineType(),
									medDto.getRemindWhen(), medDto.getOthers(), medDto.getTimes(),
									medDto.getSerialNumber(), medDto.getGenericName(), medDto.getBrandName(),
									medDto.getNameAndAddressOfTheManufacturer(), medDto.getBatchNumber(),
									medDto.getDateOfManufacturing(), medDto.getDateOfExpriy(),
									medDto.getManufacturingLicenseNumber(), medDto.getStock()));
						}
					} else {
						// New medicine (no id provided)
						updatedMedicines.add(new Medicine(UUID.randomUUID().toString(), medDto.getName(),
								medDto.getDose(), medDto.getDuration(), medDto.getDurationUnit(), medDto.getNote(),
								medDto.getFood(), medDto.getMedicineType(), medDto.getRemindWhen(), medDto.getOthers(),
								medDto.getTimes(), medDto.getSerialNumber(), medDto.getGenericName(),
								medDto.getBrandName(), medDto.getNameAndAddressOfTheManufacturer(),
								medDto.getBatchNumber(), medDto.getDateOfManufacturing(), medDto.getDateOfExpriy(),
								medDto.getManufacturingLicenseNumber(), medDto.getStock()));
					}
				}
			}
			...
			return new Response(true, responseDTO, "Prescription updated successfully", HttpStatus.OK.value());

		} catch (Exception e) {
			return new Response(false, null, "Failed to update prescription: " + e.getMessage(),
					HttpStatus.INTERNAL_SERVER_ERROR.value());
		}
	}
	...
	public Response updateMedicineById(String medicineId, MedicineDTO dto) {
		try {
			...
			if (prescriptionOpt.isEmpty()) {
				return new Response(false, null, "Medicine not found with id: " + medicineId,
						HttpStatus.NOT_FOUND.value());
			}
			...
			// find the medicine
			Medicine medicine = prescription.getMedicines().stream().filter(m -> m.getId().equals(medicineId))
					.findFirst().get();
			...
			MedicineDTO updatedDto = new MedicineDTO(medicine.getId(), medicine.getName(), medicine.getDose(),
					medicine.getDuration(), medicine.getDurationUnit(), medicine.getNote(), medicine.getFood(),
					medicine.getMedicineType(), medicine.getRemindWhen(), medicine.getTimes(), medicine.getOthers(),
					medicine.getSerialNumber(), medicine.getGenericName(), medicine.getBrandName(),
					medicine.getNameAndAddressOfTheManufacturer(), medicine.getBatchNumber(),
					medicine.getDateOfManufacturing(), medicine.getDateOfExpriy(),
					medicine.getManufacturingLicenseNumber(), medicine.getStock());
