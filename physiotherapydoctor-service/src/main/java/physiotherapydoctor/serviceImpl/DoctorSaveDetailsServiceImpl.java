...
public class DoctorSaveDetailsServiceImpl implements DoctorSaveDetailsService {
    ...
    public Response getVisitHistoryByPatient(String patientId) {
        ...
        log.info("Entered getVisitHistoryByPatient() with patientId : {}", patientId);
        ...
        try {
            ...
            log.info("Found {} visits for patientId : {}",
                    visits.size(),
                    patientId);

            long executionTime = System.currentTimeMillis() - startTime;

            log.info("getVisitHistoryByPatient() completed in {} ms",
                    executionTime);
            ...
            log.error("Exception occurred while fetching visit history for patientId : {}. Error : {}",
                    patientId,
                    e.getMessage(),
                    e);
...
//                .subServiceId(entity.getSubServiceId())
    ...
    public Response getInProgressDetails(String patientId, String bookingId) {
        ...
        log.info("Entered getInProgressDetails() with patientId : {}, bookingId : {}",
                patientId, bookingId);
        ...
        try {
            ...
            log.info("Booking fetched successfully with status : {}", booking.getStatus());

            if (!"In-Progress".equalsIgnoreCase(booking.getStatus())) {

                log.warn("Booking status is not In-Progress. Current status : {}",
                        booking.getStatus());
            ...
            log.debug("Fetching doctor details for patientId : {} and bookingId : {}",
                    patientId,
                    bookingId);
            ...
            if (visits.isEmpty()) {
                ...
                log.warn("No doctor details found for patientId : {} and bookingId : {}",
                        patientId,
                        bookingId);
                ...
                return buildResponse(
                        false,
                        null,
                        "No doctor details found for patient " + patientId +
            ...
            return buildResponse(
                    ...
                    Map.of(
                            ...
                            "savedDetails", dtos),
                    "In-Progress doctor details fetched successfully",
                    HttpStatus.OK.value());

        } catch (Exception e) {

            log.error("Exception occurred while fetching in-progress details. patientId : {}, bookingId : {}, Error : {}",
                    patientId,
                    bookingId,
                    e.getMessage(),
                    e);
