...
public class ClinicAdminFeignImpl {
	...
	public Response changePasswordFallback(String username, ChangeDoctorPasswordDTO dto, Throwable ex) {
		...
		return buildResponse(getFallbackMessage(ex));
	}
	...
	public ResponseStructure<BookingResponse> getBookingByIdFallback(String bookingId, Throwable ex) {
		...
		return buildResponseStructure(getFallbackMessage(ex));
	}
	...
	public ResponseEntity<String> getSignedUrlFallback(String fileKey, Throwable ex) {
		...
		return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
				.body("Unable to generate signed URL. Please try again later.");
	}
	...
	private String getFallbackMessage(Throwable ex) {
		...
		if (ex instanceof CallNotPermittedException)
			return "Clinic Admin Service Circuit Breaker is OPEN.";

		if (ex instanceof FeignException)
			return "Clinic Admin Service is unavailable.";

		if (ex instanceof ConnectException)
			return "Unable to connect to Clinic Admin Service.";

		if (ex instanceof SocketTimeoutException)
			return "Clinic Admin Service timed out.";

		return "Clinic Admin Service is temporarily unavailable.";
	}
}