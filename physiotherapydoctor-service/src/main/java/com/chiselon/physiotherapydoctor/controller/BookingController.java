...
import com.chiselon.physiotherapydoctor.service.BookingService;
...
public class BookingController {
	...
	@GetMapping("/appointments/patient/{clinicId}/{patientId}/{page}")
	public ResponseEntity<?> getAppointmentsByPatientId(@PathVariable String clinicId,@PathVariable String patientId,@PathVariable int page) {
		return bookingService.getAppointmentsByPatientId(clinicId,patientId,page);
	}
	...
	public ResponseEntity<?> getAllBookedServicesByDoctorId(@PathVariable String doctorId,
		...
		return bookingService.getAllBookedServicesByDoctorId(doctorId,page);
	}

	// 9. Get future appointments within next 15 days
	@GetMapping("/getFutureDoctorappointmentsByDoctorId/{doctorId}/{page}")
	public ResponseEntity<?> getDoctorFutureAppointments(@PathVariable String doctorId,
														 @PathVariable int page) {

		return bookingService.getDoctorFutureAppointments(doctorId,page);
	}
